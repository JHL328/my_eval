...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir", type=str, default=None, help="The root directory to save the results, like /mnt/sharefs/users/haolong.jia/result/humaneval")
    ...
    for model_path, model_name in Model_map.items():
        ...
        all_results, dataset_name = load_all_results(result_dir)
        if dataset_name:
        ...
        dataset_str = dataset_name.upper() if dataset_name else args.dataset.upper()
        with open(metrics_txt_path, "w") as f:
            f.write(f"{dataset_str} Base pass@k (robust):\n")
            for k in ks:
                if f"pass@{k}" in base_pass_at_k:
                    f.write(f"pass@{k}: {base_pass_at_k[f'pass@{k}']:.4f}\n")
            f.write(f"\n{dataset_str}+ Plus pass@k (robust):\n")
            for k in ks:
        ...
        with open(summary_file, "w") as f:
            ...
            f.write(f"\n{dataset_str} (base tests):\n")
            for k, v in base_pass_at_k.items():
                f.write(f"  {k}: {v:.3f}\n")
            f.write(f"\n{dataset_str}+ (base + extra tests):\n")
