...
def infer(args):
    ...
    out_file = f'{args.output_dir}/{args.data_name}/{model_name_or_path_re}/{out_file_prefix}_k{args.n_sampling}_s{args.start_idx}_e{args.end_idx}.jsonl'
    
    
    # if os.path.exists(out_file):
    #     print(f"Completely same name file({out_file}) exist, skip generation, save file and check correct")
    #     return
    os.makedirs(f'{args.output_dir}/{args.data_name}/{model_name_or_path_re}', exist_ok=True)
    os.makedirs(f'{args.completions_save_dir}/{args.data_name}/{model_name_or_path_re}', exist_ok=True)
    ...
    for cur_generation_epoch in range(generation_epoch):
        completions_save_file = f'{args.completions_save_dir}/{args.data_name}/{model_name_or_path_re}/{out_file_prefix}_k{args.n_sampling}_s{args.start_idx}_e{args.end_idx}_gen_round{cur_generation_epoch}.pkl'
    ...
    if pass_at_k_list:
        ...
        results[f"pass@{k}"] = f"{average_pass_at_k:.4f}"
    else:
        print(f"Pass@1: {correct_cnt}/{len(examples)} = {correct_cnt / len(examples):.4f}")
        results[f"pass@1"] = f"{correct_cnt / len(examples):.4f}"
